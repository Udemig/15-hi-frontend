...
const Card: FC<Props> = ({ note }) => {
  return (
    ...
    >
      ...
      {/* İçerik */}
      <p className="line-clamp-2 text-text-secondary text-sm mb-4 flex-1">{note.content}</p>
      ...
      <div className="flex flex-wrap gap-2 mt-auto pt-2 border-t border-border">
        {note.tags.slice(0, 4).map((tag, key) => (
          <span
            key={key}
            className="bg-primary text-text-primary px-2 py-0.5 mt-2 rounded-full inline-flex items-center"
          >
            {tag}
          </span>
        ))}
