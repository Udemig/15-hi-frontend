        ...
        <tbody>
          {users.map((user, key) => (
            <tr className="hover:bg-zinc-50/70 transition-colors">
              ...
              <td>
                <span className="items-center justify-center inline-flex min-w-7 px-2 py-0.5 bg-blue-50 text-blue-700 text-xs font-semibold ring-1 ring-inset ring-blue-600/20 rounded-full">
                  {user.orders.length}
                </span>
              </td>
              <td>
                <div className="flex gap-2 justify-center">
                  <Link href={`?userId=${user.id}`} className="button hover:bg-zinc-100">
